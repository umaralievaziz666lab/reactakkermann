...
export default function BottomNav({ section, onSection, onCreate, notifBadge, profileBadge }) {
  const navStyle = (active) => ({
    ...
    background: 'none', border: 'none', cursor: 'pointer', gap: 2, position: 'relative',
    padding: '10px 0', minHeight: 54, flex: 1,
  ...
  return (
    <nav style={{
      ...
      background: 'var(--navy)', borderTop: '3px solid var(--red)',
      display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)',
      boxShadow: '0 -4px 20px rgba(0,0,0,.2)',
      paddingBottom: 'env(safe-area-inset-bottom, 0px)',
    }}>
      <button style={navStyle(section === 'feed')} onClick={() => onSection('feed')}>
      ...
      <button style={navStyle(section === 'notif')} onClick={() => onSection('notif')}>
        ...
        {notifBadge > 0 && (
          <Badge>{notifBadge > 9 ? '9+' : notifBadge}</Badge>
        )}
      </button>
      ...
      <button style={navStyle(section === 'news')} onClick={() => onSection('news')}>
      ...
      <button style={navStyle(section === 'profile')} onClick={() => onSection('profile')}>
...
function NavLabel({ children }) {
  return (
    <span style={{ fontSize: 10, fontWeight: 700, fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '.05em' }}>
      {children}
    </span>
  )
}
