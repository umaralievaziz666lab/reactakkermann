...
export default function AdminRequests({ adminUser, showToast, onBadgeUpdate }) {
  ...
  async function exportExcel() {
    ...
    try {
      const rows = [
        ['ID', 'Тип', 'Описание', 'Автор', 'Участок', 'Статус', 'Уровень риска', 'Назначено', 'Дедлайн', 'Дата создания'],
        ...filtered.map(r => [
          pad(r.id),
      ...
      // Build CSV (works without external libs)
      ...
      a.href = url
      a.download = `akkermann-pulse-${new Date().toLocaleDateString('ru').replace(/\./g,'-')}.csv`
      a.click()
  ...
  async function exportPDF() {
    ...
    try {
      ...
      const rows = filtered.slice(0, 100) // PDF limit
      ...
      const html = `
        ...
        <head>
          ...
          <style>
            ...
            h1 { font-size: 18px; color: #f53d2d; margin-bottom: 4px; }
            ...
            .idea { color: #059669; font-weight: bold; }
            .badge { padding: 2px 6px; border-radius: 10px; font-size: 9px; font-weight: bold; }
            @media print { body { margin: 0; } }
          </style>
        </head>
        <body>
          <h1>AKKERMANN PULSE — Отчёт по заявкам</h1>
          <div class="sub">Дата: ${date} | Всего: ${rows.length} заявок${rows.length < filtered.length ? ` (показано первые 100 из ${filtered.length})` : ''}</div>
          <table>
            <thead>
              <tr>
                <th>ID</th><th>Тип</th><th>Описание</th><th>Автор</th>
                <th>Участок</th><th>Статус</th><th>Дата</th>
              </tr>
            </thead>
            <tbody>
              ${rows.map(r => `
                <tr>
                  ...
                  <td style="max-width:200px">${(r.description || '').slice(0, 100)}${(r.description||'').length > 100 ? '…' : ''}</td>
                  <td>${r.anonymous ? 'Аноним' : (r.author || '—')}</td>
                  <td>${r.location || '—'}</td>
                  <td>${STATUS_MAP[r.status]?.label || r.status || '—'}</td>
      ...
      showToast(`✅ PDF готов — используй "Сохранить как PDF" при печати`)
    } catch(e) {
      ...
      <div style={{ flex: selected ? '0 0 460px' : 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
            style={{ ...selStyle, color: dateFrom ? '#0f1c2c' : '#8fa0ae' }} title="Дата от" />
          <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)}
            style={{ ...selStyle, color: dateTo ? '#0f1c2c' : '#8fa0ae' }} title="Дата до" />
          ...
          {(dateFrom || dateTo || search || statusFilter !== 'all' || typeFilter !== 'all') && (
            <button onClick={() => { setSearch(''); setStatusFilter('all'); setTypeFilter('all'); setDateFrom(''); setDateTo('') }}
              style={{ padding: '7px 10px', borderRadius: 9, border: '1px solid #fecaca', background: '#fef2f2', color: '#ef4444', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
              ✕ Сброс
            </button>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 10, alignItems: 'center' }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
            <button onClick={exportExcel} disabled={exporting || filtered.length === 0}
              style={{ padding: '7px 14px', borderRadius: 3, border: 'none', background: '#16a34a', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, opacity: exporting ? .6 : 1, fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '.04em' }}>
              📊 Excel / CSV
            </button>
            <button onClick={exportPDF} disabled={exporting || filtered.length === 0}
              style={{ padding: '7px 14px', borderRadius: 3, border: 'none', background: '#dc2626', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, opacity: exporting ? .6 : 1, fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '.04em' }}>
              📄 PDF
            </button>
          </div>
        </div>
        ...
        <div style={{ flex: 1, background: '#fff', borderRadius: 3, border: '1px solid #d1cfc9', overflow: 'hidden', display: 'flex', flexDirection: 'column', minWidth: 300 }}>
          ...
          <div style={{ overflowY: 'auto', flex: 1, padding: 16 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              {selected.deadline && (
                <div style={{ fontSize: 12, marginTop: 4, color: new Date(selected.deadline) < new Date() ? '#dc2626' : '#5a7080', fontWeight: new Date(selected.deadline) < new Date() ? 700 : 400 }}>
                  ⏰ Дедлайн: {fmtDate(selected.deadline)}
                  {new Date(selected.deadline) < new Date() ? ' — ПРОСРОЧЕНО!' : ''}
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <Label>Статус</Label>
                <select value={editFields.status || selected.status} onChange={e => setEditFields(f => ({ ...f, status: e.target.value }))}
              ...
              <div>
                ...
                <select value={editFields.assigned_to} onChange={e => setEditFields(f => ({ ...f, assigned_to: e.target.value }))}
                  style={{ width: '100%', ...selStyle }}>
              ...
              <div>
                <Label>Дедлайн</Label>
                <input type="datetime-local" value={editFields.deadline}
              ...
              <div>
                ...
                <textarea value={editFields.admin_comment}
                  onChange={e => setEditFields(f => ({ ...f, admin_comment: e.target.value }))}
