...
export default function AdminRequests({ adminUser, showToast, onBadgeUpdate }) {
  ...
  async function updateRequest(id, updates, logAction) {
    ...
    if (updates.status && req?.author_id) {
      ...
      const msg = `Заявка ${pad(id)} → ${stLabel}${updates.admin_comment ? `\nКомментарий: ${updates.admin_comment}` : ''}`
      await notifyUser(supabase, req.author_id, `🔄 Статус изменён: ${stLabel}`, msg, 'status_update')
    }

    // 🔔 Уведомление исполнителю если назначили
    if (updates.assigned_to && updates.assigned_to !== req?.assigned_to) {
      const assignedUser = users.find(u => u.name === updates.assigned_to)
      if (assignedUser) {
        await notifyUser(
          supabase, assignedUser.id,
          '👤 Вам назначена заявка',
          `${pad(id)} — ${(req?.description || '').slice(0, 100)}\nУчасток: ${req?.location || '—'}`,
          'status_update'
        )
      }
    }

    // 🔔 Уведомление о дедлайне если поставили
    if (updates.deadline && !req?.deadline) {
      if (req?.author_id) {
        await notifyUser(
          supabase, req.author_id,
          '⏰ Установлен дедлайн',
          `${pad(id)}: срок выполнения ${fmtDate(updates.deadline)}`,
          'deadline'
        )
      }
    }
  ...
  async function deleteRequest(id) {
    if (!can(role, 'delete_request')) { showToast('Только администратор может удалять', 'error'); return }
    if (!confirm(`Удалить заявку ${pad(id)}?`)) return
  ...
  async function exportExcel() {
    ...
    const rows = [
      ...
      ...filtered.map(r => [
        pad(r.id), r.type==='risk'?'Риск':'Идея', r.description||'',
        r.anonymous?'Аноним':(r.author||''), r.location||'',
        STATUS_MAP[r.status]?.label||'', r.risk_urgency?`Уровень ${r.risk_urgency}`:'',
        r.assigned_to||'', r.deadline?fmtDate(r.deadline):'', fmtDate(r.date||r.created_at),
      ])
    ]
  ...
  async function exportPDF() {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8">
      <style>body{font-family:Arial,sans-serif;font-size:11px;margin:20px}h1{color:#f53d2d}
      table{width:100%;border-collapse:collapse}th{background:#0f1c2c;color:white;padding:6px 8px;text-align:left;font-size:10px}
      td{padding:5px 8px;border-bottom:1px solid #eee}tr:nth-child(even) td{background:#f9f9f9}</style>
      </head><body><h1>AKKERMANN PULSE — Отчёт</h1>
      <p style="color:#666;font-size:10px">${date} | ${filtered.length} заявок | ${adminUser.name}</p>
      ...
      <tbody>${filtered.slice(0,100).map(r=>`<tr>
        <td style="color:#f53d2d;font-family:monospace">${pad(r.id)}</td>
        ...
        <td>${r.location||'—'}</td><td>${STATUS_MAP[r.status]?.label||'—'}</td>
        <td>${fmtDate(r.date||r.created_at)}</td></tr>`).join('')}
          ...
          {can(role,'export_data') && (
            <div style={{ marginLeft:'auto', display:'flex', gap:6 }}>
              <button onClick={exportExcel} disabled={exporting||!filtered.length}
                style={{ padding:'7px 14px', borderRadius:3, border:'none', background:'#16a34a', color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif" }}>📊 Excel</button>
              <button onClick={exportPDF} disabled={exporting||!filtered.length}
                style={{ padding:'7px 14px', borderRadius:3, border:'none', background:'#dc2626', color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif" }}>📄 PDF</button>
            </div>
          )}
        </div>
          ...
          <div style={{ overflowY:'auto', flex:1, padding:16 }}>
            <div style={{ marginBottom:14 }}>
              <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:8 }}>
                ...
                <span style={{ fontSize:11, fontWeight:700, padding:'2px 8px', borderRadius:999, background:`${(STATUS_MAP[selected.status]||STATUS_MAP.new).color}18`, color:(STATUS_MAP[selected.status]||STATUS_MAP.new).color }}>
              ...
              <div style={{ fontSize:12, color:'#5a7080', marginBottom:4 }}>
                👤 {selected.anonymous?'Аноним':selected.author} · 📍 {selected.location||'—'} · 🕐 {fmtDate(selected.date||selected.created_at)}
              </div>
              {selected.assigned_to && (
                <div style={{ fontSize:12, color:'#3b82f6', marginBottom:4 }}>👷 Исполнитель: <strong>{selected.assigned_to}</strong></div>
              )}
              {selected.deadline && (
                <div style={{ fontSize:12, fontWeight:700, color:new Date(selected.deadline)<new Date()?'#dc2626':'#5a7080' }}>
